...
test("core library buy/sell balance honors natural vs artificial mode", async () => {
  const trades = [
    makeTrade({
      id: "mix-1",
      entryIso: "2025-03-01T00:00:00Z",
    ...
    makeTrade({
      id: "mix-4",
      entryIso: "2025-03-01T06:00:00Z",
      exitIso: "2025-03-01T07:00:00Z",
      side: "Short",
      result: "Loss",
      pnlUsd: -110,
      pnlPct: -0.22,
    ...
    makeTrade({
      id: "mix-5",
      entryIso: "2025-03-01T08:00:00Z",
      exitIso: "2025-03-01T09:00:00Z",
      side: "Long",
      neighborVector: [0, 0, 0, 0, 0, 0]
    })
  ];

  const artificialPayload = await postPanelAnalytics({
    panelSourceTrades: trades,
    panelLibraryPoints: [],
    panelBacktestFilterSettings: baseFilterSettings({
      selectedAiLibraries: ["core"],
  ...
  const artificialTrade = artificialPayload.timeFilteredTrades.find(
    (trade: { id: string }) => trade.id === "mix-5"
  );
  assert.ok(artificialTrade, "expected the latest trade in the artificial payload");
  const artificialNeighborSides = artificialTrade.entryNeighbors.map(
    (neighbor: { t?: { side?: string } }) => neighbor.t?.side
  );
  assert.ok(artificialNeighborSides.length > 0, "expected artificial neighbors to be present");
  assert.deepEqual(new Set(artificialNeighborSides), new Set(["Long"]));

  const naturalPayload = await postPanelAnalytics({
    panelSourceTrades: trades,
    panelLibraryPoints: [],
    panelBacktestFilterSettings: baseFilterSettings({
      selectedAiLibraries: ["core"],
  ...
  const naturalTrade = naturalPayload.timeFilteredTrades.find(
    (trade: { id: string }) => trade.id === "mix-5"
  );
  assert.ok(naturalTrade, "expected the latest trade in the natural payload");
  const naturalNeighborSides = naturalTrade.entryNeighbors.map(
    (neighbor: { t?: { side?: string } }) => neighbor.t?.side
  );
  assert.ok(naturalNeighborSides.includes("Long"));
  assert.ok(naturalNeighborSides.includes("Short"));
});
