...
test("online anti-cheat excludes future canonical library points from neighbors", async () => {
  const trades = [
    makeTrade({
      id: "live-1",
      entryIso: "2025-03-01T00:00:00Z",
  ...
  const libraryPoints = [
    {
      uid: "lib|alpha|past|0",
      libId: "alpha",
      metaTime: "2025-02-28T22:00:00Z",
      readyTime: "2025-02-28T23:00:00Z",
      metaPnl: 100,
      metaOutcome: "Win",
      metaSession: "London",
      dir: 1,
    ...
    {
      uid: "lib|alpha|future|1",
      libId: "alpha",
      metaTime: "2025-03-01T05:00:00Z",
      readyTime: "2025-03-01T05:30:00Z",
      metaPnl: 100,
      metaOutcome: "Win",
      metaSession: "London",
      dir: 1,
  ...
  const payload = await postPanelAnalytics({
    ...
    panelBacktestFilterSettings: baseFilterSettings({
      antiCheatEnabled: true,
      validationMode: "off",
      selectedAiLibraries: ["alpha"],
      selectedAiLibrarySettings: {
        alpha: { weight: 100, maxSamples: 1000 }
      }
    }),
    panelConfidenceGateDisabled: true,
    panelEffectiveConfidenceThreshold: 0,
  ...
  assert.ok(neighborIds.includes("lib|alpha|past|0"));
  assert.equal(neighborIds.includes("lib|alpha|future|1"), false);
  assert.equal(stampedTrade.closestClusterUid, "lib|alpha|past|0");
});

test("online anti-cheat excludes canonical library points that are not ready yet", async () => {
  const trades = [
    makeTrade({
      id: "live-1",
      entryIso: "2025-03-01T00:00:00Z",
  ...
  const libraryPoints = [
    ...
    {
      uid: "lib|alpha|not-ready|1",
      libId: "alpha",
      metaTime: "2025-03-01T01:00:00Z",
      readyTime: "2025-03-01T04:00:00Z",
      metaPnl: 100,
      metaOutcome: "Win",
      metaSession: "London",
      dir: 1,
  ...
  assert.ok(neighborIds.includes("lib|alpha|ready|0"));
  assert.equal(neighborIds.includes("lib|alpha|not-ready|1"), false);
  assert.equal(stampedTrade.closestClusterUid, "lib|alpha|ready|0");
});
